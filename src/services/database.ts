...
export class DatabaseService {
  ...
  async init() {
    ...
    try {
      ...
      if (!tradeLogColumns.includes('exitOrderId')) {
        console.log('Migrating database: Adding exitOrderId column');
        this.db.exec(`ALTER TABLE trade_logs ADD COLUMN exitOrderId TEXT`);
      }
    ...
    try {
      const stmt = this.db.prepare(`
        INSERT OR REPLACE INTO trade_logs (
          id, accountId, symbol, side, leverage, amount, entryPrice, exitPrice, pnl, fee, fundingFee, fundingFeeCheckedCount, profitRate, kBestChange, mValue, realA, openTime, closeTime, status, exitOrderId
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      stmt.run(
        ...
        log.kBestChange || 0, log.mValue || 0, log.realA || 0,
        log.openTime, log.closeTime, log.status, log.exitOrderId || null
      );
    } catch (error) {
