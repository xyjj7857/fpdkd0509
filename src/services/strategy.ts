...
export class StrategyEngine {
        ...
        if (isFilled) {
          ...
          if (openTrade) {
            ...
            if (isClosingOrder) {
              ...
              // 方案一：支持分笔记录平仓 (通过传入订单ID和数量)
              this.confirmPositionClosed(symbol, parseFloat(order.L), order.T, order.i.toString(), parseFloat(order.q));
            }
          }
        }
      } else {
  ...
  private async confirmPositionClosed(symbol: string, exitPrice?: number, exitTime?: number, exitOrderId?: string, exitQty?: number) {
    // 1. 记录平仓时间用于冷却期
    ...
    const openTrade = this.tradeLogs.find(t => t.symbol === symbol && t.status === 'OPEN');
    if (openTrade) {
      this.addLog('订单', `[账户] binance仓单成交完成 (确认平仓): ${symbol}${exitQty ? ` (数量: ${exitQty})` : ''}`, 'success');
      ...
      try {
        ...
        // 判定是否是部分平仓 (由 WebSocket 触发且数量小于当前持仓记录，且有订单ID)
  ...
  private async runStage2(isManual: boolean = false): Promise<boolean> {
        ...
        const results = await Promise.all(batch.map(async (symbol) => {
          try {
            ...
            return {
              ...
              k2: ((current15 - open15) / open15) * 100,
              a: ((high15 - current15) / current15) * 100,
              amp: ((high15 - low15) / low15) * 100,
              status: 'Stage 1 Pass',
    ...
    for (const item of sharedTable) {
      ...
      for (const [key, config] of Object.entries(conditions) as [string, any][]) {
        ...
        if (key === 'a') {
          ...
          if (config.mode === 'fixed') {
             const [min, max] = config.fixedRange;
             if (aVal < min || aVal > max) { passed = false; failReason = `A:${aVal.toFixed(2)}%`; break; }
          } else {
             const kVal = item.k2; if (kVal === 0) { passed = false; break; }
             const relativeVal = (aVal / kVal) * 100;
             const [min, max] = config.relativeRange;
             if (relativeVal < min || relativeVal > max) { passed = false; failReason = `RelA:${relativeVal.toFixed(2)}%`; break; }
          }
        } else if (key !== 'k5') {
          const val = item[key === 'k2' ? 'k2' : 'volume15'];
          const [min, max] = config.range;
          if (val < min || val > max) { passed = false; failReason = `${key.toUpperCase()}:${val.toFixed(2)}`; break; }
        }
      }
      ...
      accountResults.push({
        ...
        k2: item.k2.toFixed(2), a: item.a.toFixed(2), m: (item.volume15 / 1000000).toFixed(2), amp: item.amp.toFixed(2)
      });
    }
    ...
    accountResults.sort((a,b) => {
      ...
      if (!a.isPass && b.isPass) return 1;
      if (preferredMode === 'k2') return parseFloat(b.k2) - parseFloat(a.k2);
    ...
    for (let i = accountResults.length - 1; i >= 0; i--) {
      ...
      const logMsg = `${r.symbol}: K=${r.k2}%, A=${r.a}%, M=${r.m}M. ${r.isPass ? '通过' + (r.isPreferred ? ' (优选)' : '') : '未通过: ' + r.reason}`;
  ...
  private async handleTPAndSL(symbol: string, quantity: string, scanStartTime: number, symbolInfo: any, orderId: string, executionData?: { avgPrice: number, qty: number }) {
    ...
    try {
      ...
      // 止盈价计算
      let theoreticalTP1: number, theoreticalTP2: number;
      if (this.settings.order.tpMode === 'fixed') {
        const tpFixed = this.settings.order.tpFixed / 100;
        theoreticalTP1 = kBestClose * (1 + tpFixed);
        theoreticalTP2 = theoreticalTP1;
      } else {
        const tpRatio1 = (this.settings.order.tpRatio1 || this.settings.order.tpRatio) / 100;
        const tpRatio2 = (this.settings.order.tpRatio2 || this.settings.order.tpRatio) / 100;
        theoreticalTP1 = kBestClose * (1 + kBestChange * tpRatio1);
        theoreticalTP2 = kBestClose * (1 + kBestChange * tpRatio2);
      }
      ...
      const safeTP1 = Math.max(theoreticalTP1, entryPrice * 1.0001);
      const safeTP2 = Math.max(theoreticalTP2, entryPrice * 1.0001);
      ...
      const formattedTP1 = (Math.floor((safeTP1 + epsilon) / tickSize) * tickSize).toFixed(pricePrecision);
      const formattedTP2 = (Math.floor((safeTP2 + epsilon) / tickSize) * tickSize).toFixed(pricePrecision);
      // 止损向上取整 (ceil)：确保更安全止损
      const formattedSL = (Math.ceil((safeSL - epsilon) / tickSize) * tickSize).toFixed(pricePrecision);

      this.addLog('下单', `[最高] 获取k优开计算完成: ${symbol} [${targetStartStr} - ${targetEndStr}] - 入场价: ${entryPrice}, TP1: ${formattedTP1}, TP2: ${formattedTP2}, SL: ${formattedSL}`, 'success');
      ...
      const actualTotalQuantity = Math.abs(this.currentPosition.amount);
      
      // 根据配置的仓位比例计算两部分数量，确保精度对齐
      const tpQtyRatio1 = (this.settings.order.tpQtyRatio1 ?? 50) / 100;
      const qty1Val = Math.floor((actualTotalQuantity * tpQtyRatio1) / stepSize) * stepSize;
      const qty2Val = Number((actualTotalQuantity - qty1Val).toFixed(qtyPrecision));
      
      const qty1 = qty1Val.toFixed(qtyPrecision);
      const qty2 = qty2Val.toFixed(qtyPrecision);
      const actualQuantity = actualTotalQuantity.toFixed(qtyPrecision);

      this.addLog('下单', `检测到持仓: ${symbol}, 总数: ${actualQuantity} (拆分: ${qty1} (@${tpQtyRatio1*100}%) + ${qty2}), 准备分批挂单...`, 'info');
      ...
      if (currentLimitCount < targetLimitCount && this.settings.order.tpEnabled !== false) {
        ...
        if (parseFloat(qty1) > 0) {
          ...
          this.addLog('下单', `[阶段1] limit挂单完成: ${symbol}, 数量: ${qty1}, 价格: ${formattedTP1}, 订单ID: ${tpOrder1.orderId}`, 'success', tpOrder1);
        }

        // 第二批
        if (parseFloat(qty2) > 0) {
          const tpOrder2 = await this.binance.placeOrder({
            symbol,
            side: 'SELL',
            type: 'LIMIT',
            quantity: qty2,
          ...
          this.addLog('下单', `[阶段2] limit挂单完成: ${symbol}, 数量: ${qty2}, 价格: ${formattedTP2}, 订单ID: ${tpOrder2.orderId}`, 'success', tpOrder2);
        }
      ...
      if (!hasAlgo && this.settings.order.slEnabled !== false) {
        try {
          ...
          // 如果当前价格已经低于（或等于）止损价，说明止损已被穿透
          // 对于多单来说，currentPrice <= slPrice 时，STOP_MARKET 会立即触发
          if (currentPrice <= slPrice) {
            this.addLog('下单', `[预检] 当前价 ${currentPrice} 已低于/触及止损价 ${slPrice}，跳过挂单并执行紧急市价平仓`, 'warning');
            await this.closeCurrentPosition();
            return;
          }
